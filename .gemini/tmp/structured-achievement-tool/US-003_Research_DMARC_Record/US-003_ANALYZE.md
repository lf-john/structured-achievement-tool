...
## Story: Research DMARC Record for Email Authentication

**ID:** US-003

**Description:** Generate and document a DMARC policy record for `logicalfront.net` starting with a monitoring policy (`p=none`) to collect reports before enforcing. Include an aggregate reporting address (`rua=mailto:dmarc-reports@logicalfront.net`). The host should be `_dmarc.logicalfront.net` and `pct=100`.

**Acceptance Criteria:**
1. Documented DMARC TXT record for `_dmarc.logicalfront.net` with `v=DMARC1; p=none; rua=mailto:dmarc-reports@logicalfront.net; pct=100`.
