...
Components:
  ...
  - src.core.checkpoint_manager: Used for DB operations.
...
Test Cases:
  ...
  2. [AC 2] -> test_checkpoint_should_reflect_completed_and_pending_stories
     - Mock read_checkpoint to return an existing checkpoint with some completed/pending stories.
     - Execute a new story from the pending list.
     - Verify write_checkpoint was called with updated completed/pending lists.

Edge Cases:
  - Story execution fails: Verify write_checkpoint is NOT called (as per AC "after every successful story execution").
  - Checkpoint doesn't exist yet: Verify a new one is created.
"""
...
async def test_should_write_checkpoint_after_successful_story_execution():
    ...
    working_dir = "/tmp/sat_test_us_002_ac1"
    os.makedirs(working_dir, exist_ok=True)
    
    story = {"id": "STORY-1", "title": "Test Story", "type": "development"}
    task_id = "TASK-1"
...
async def test_checkpoint_should_reflect_completed_and_pending_stories():
    ...
    working_dir = "/tmp/sat_test_us_002_ac2"
    os.makedirs(working_dir, exist_ok=True)
...
async def test_should_not_write_checkpoint_on_failure():
    ...
    working_dir = "/tmp/sat_test_us_002_fail"
    os.makedirs(working_dir, exist_ok=True)
