...
def get_mautic_cron_schedule(run_shell_command_func):
    ...
    if result.get("exit_code", 0) != 0:
        ...
        user_crontabs_list = run_shell_command_func(command, description=f"List user crontabs in container {container_id}")
        if user_crontabs_list.get("exit_code", 0) == 0 and user_crontabs_list.get("output", "").strip():
            ...
            for user_cron_file in user_crontabs_list.get("output", "").splitlines():
                ...
                user_cron_content = run_shell_command_func(command, description=f"Read user crontab for {user_cron_file} in container {container_id}")
                if user_cron_content.get("exit_code", 0) == 0 and "mautic:emails:send" in user_cron_content.get("output", ""):
...
def get_mautic_mailer_spool_msg_limit(run_shell_command_func):
    ...
    for path in search_paths:
        ...
        result = run_shell_command_func(command, description=f"Read Mautic config file {path} in container {container_id}")
        if result.get("exit_code", 0) == 0:
